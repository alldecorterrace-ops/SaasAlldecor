...
async function main() {
  ...
  try {
    console.log(
      JSON.stringify(await stageEstimates(db, snapshot, values["company-id"])),
    );
  } finally {
    await db.close();
  }
...
main().catch(() => {
  console.error(
    "Ensayo detenido. Revisa la carpeta privada, empresa y conflictos de origen. No se conecta al SaaS ni sobrescribe registros históricos.",
  );
  process.exitCode = 1;
});
