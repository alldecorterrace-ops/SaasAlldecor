...
Fecha de inicio: 18 de septiembre de 2026. Seguimiento actualizado: 25 de septiembre de 2026. Destino: https://app.alldecorpatio.com.
...
acredita por sí solo la pantalla completa. La continuación conectó el mapa y
verificó UI, permisos, ubicaciones y descarga en staging `8d936f0`, esquema 032,
315 pruebas y CI completo. Véase la
[auditoría del mapa comercial](AUDITORIA-MAPA-COMERCIAL-20260925.md).

Continuación `b4377d3`: ubicación automática con respuestas sintéticas, guardado,
reapertura, reintento y revocación comprobados en staging; conexión real de un
ZIP público y caché verificadas por separado. 324 pruebas y los tres jobs de CI
aprobados. Esquema 032 sin cambios. Véase la
...
La entrega `6866a73` y la migración 029 corrigen el acceso de un
...
solo tras recálculo explícito, generando otro estimado sin alterar el anterior.
Tras inventario y autorización expresa, se eliminaron cuatro entregas antiguas
...
La entrega de staging `685b5da` extiende la conservación de borradores a
...
El 25 de septiembre quedó activa `7472d42`, con 30 migraciones. Se corrigió un
defecto reproducido en Inventario: llegar a saldo cero permitía cambiar la unidad
y reinterpretar visualmente todos los movimientos anteriores. La protección
aditiva 030 y el mensaje del formulario se comprobaron con la misma sesión;
el rechazo no cambió registro ni movimientos y corregir la ubicación sí persistió.
Pasaron lint, tipos, 282 pruebas, compilación y los tres trabajos de
[CI](https://github.com/alldecorterrace-ops/SaasAlldecor/actions/runs/36148761866).
La [auditoría de Inventario](AUDITORIA-STAGING-20260925.md) separa evidencia de UI,
base, permisos y emulación móvil. Anterior `685b5da`, dependencia `b149bee`;
las sobrantes `6866a73` y `f29072d` siguen conservadas. Producción no recibió
esta publicación ni 026–030. No se ha cerrado ninguno de los seis puntos.

La publicación posterior del 25 de septiembre dejó activa `4293666`, anterior
`7472d42` y dependencia compartida `b149bee`. La migración aditiva 031 amplía
la búsqueda de Permisos con fechas inclusivas y acceso restringido al nombre
de proyecto; no modifica registros. Pasaron lint, tipos, 287 pruebas, build
y los tres trabajos de [CI](https://github.com/alldecorterrace-ops/SaasAlldecor/actions/runs/36150779413).
La [auditoría de Permisos e Instalaciones](AUDITORIA-PERMISOS-INSTALACIONES-20260925.md)
documenta los recorridos sintéticos, rechazo de superposición, fechas, estados,
adjunto, revocación y verificaciones posteriores a la publicación. El auditor
volvió a ventas; su estimado anterior conserva revisión e importe. Producción
mantiene `3c0c412` y no recibió 026–031. No hubo borrados; quedan identificadas
`685b5da`, `6866a73` y `f29072d` para revisar y confirmar la retención.

La siguiente entrega `a8e1dd9` conserva 31 migraciones y corrige la impresión
manual: utiliza la revisión guardada, sin borradores rechazados del formulario.
Pasaron 288 pruebas, lint, tipos, build y CI. La auditoría autenticada comprobó
aprobaciones, revisiones, archivos y bloqueo del manual archivado; zonas probó
coordenadas, estado, filtros y revocación. La vista imprimible y mensajes de
coordenadas se repitieron tras publicar. Véanse detalles y diferencias pendientes
con ADT en [Manuales y Zonas](AUDITORIA-MANUALES-ZONAS-20260925.md).
Anterior `4293666`, dependencia `b149bee`; producción sigue en `3c0c412`.
Las sobrantes `7472d42`, `685b5da`, `6866a73` y `f29072d` permanecen sin borrar.
Los seis puntos continúan abiertos.
